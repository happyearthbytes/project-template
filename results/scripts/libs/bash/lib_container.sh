...
__build_container_from_dir()
{
  ...
  for item in ${container[@]} ; do
    ...
    local image_requirements=$(sed -n '/requirements:/ s/[^:]*:\s\+//p' ${item}/config.yml)
    ...
    local image_name=$(sed -n '/image_name:/ s/[^:]*:\s\+//p' ${this_item}/config.yml)
    local image_tag=$(sed -n '/image_tag:/ s/[^:]*:\s\+//p' ${this_item}/config.yml)
